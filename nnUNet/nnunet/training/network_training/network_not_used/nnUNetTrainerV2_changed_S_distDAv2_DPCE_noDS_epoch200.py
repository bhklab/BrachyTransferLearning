...
class nnUNetTrainerV2_changed_S_distDAv2_DPCE_noDS_epoch200(nnUNetTrainerV2):
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        '''only segment sigmoid'''
        # target_sub = target.copy()
        target_sub = torch.where(target!=3,torch.tensor(0.), target)
        target_sub = torch.where(target_sub==3,torch.tensor(1.), target_sub)
        '''only let dist map for sigmoid remain'''
