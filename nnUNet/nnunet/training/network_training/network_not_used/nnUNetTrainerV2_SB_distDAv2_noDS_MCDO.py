...
class nnUNetTrainerV2_SB_distDAv2_noDS_MCDO(nnUNetTrainerV2):
    ...
    def initialize_network(self, mcdo=-1):
        ...
        # use dropout using training for uncertainty estimation 
        # if mcdo != -1:
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
                                                         ...
                                                         verbose: bool = True, mixed_precision=True, mcdo: int = -1) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        ret = super().predict_preprocessed_data_return_seg_and_softmax(data,
                                                                       ...
                                                                       verbose=verbose,
                                                                       mixed_precision=mixed_precision, mcdo=mcdo)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        '''only segment small bowel'''
        # target_sub = target.copy()
        target_sub = torch.where(target!=4,torch.tensor(0.), target)
        target_sub = torch.where(target_sub==4,torch.tensor(1.), target_sub)
        # '''only let dist map for small bowel remain'''
