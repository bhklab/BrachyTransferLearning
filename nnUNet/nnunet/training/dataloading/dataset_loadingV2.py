...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        ...
        num_color_channels = case_all_data.shape[0]
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        seg_shape = (self.batch_size, num_seg, *self.patch_size)
        # dist_shape = (self.batch_size, num_color_channels, *self.patch_size)
    ...
    def generate_train_batch(self):
        ...
        case_properties = []
        # dist = np.zeros(self.dist_shape, dtype=np.float32)
        for j, i in enumerate(selected_keys):
            ...
            # '''calculate dist map before padding'''
            # gt = case_all_data[-1:]
            # gt_copy = gt.copy()
            # gt_copy = np.asarray(gt_copy)
            # dist_sub = compute_edts_forPenalizedLoss(gt_copy)
            ...
            '''calculate dist map based on seg and concatenate to data (img)'''
            data_ori = case_all_data[:-1]
            seg_ori = case_all_data[-1:]
            seg_ori_copy = seg_ori.copy()   
            dist = compute_edts_forPenalizedLoss(seg_ori_copy) 
            dist = dist[np.newaxis,:,:,:] # add one dimension at axis=0
            data_dist = np.concatenate((data_ori, dist), axis=0)
...
if __name__ == "__main__":
    ...
    p = '/cluster/home/nir/nnUNetFrame/DATASET/nnUNet_preprocesed/Task095_gyn/nnUNetData_plans_v2.1_stage0'
    plan_p = '/cluster/home/nir/nnUNetFrame/DATASET/nnUNet_preprocesed/Task095_gyn/nnUNetPlansv2.1_plans_3D.pkl'
    dataset = load_dataset(p)
    # with open(join(join(preprocessing_output_dir, t), "plans_stage1.pkl"), 'rb') as f:
    with open(plan_p, 'rb') as f:
        plans = pickle.load(f)
    unpack_dataset(p)
