...
def get_distDAv2_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
    ...
    '''Here the dist map is moving to another key so it does not get intensity/Gaussian transform'''
    tr_transforms.append(AppendChannelsTransform("data", "dist", [1], remove_from_input=True))
    ...
    # add mirror to dist as well (before channel split)
    # if params.get("do_mirror") or params.get("mirror"):
    #     tr_transforms.append(MirrorTransform(params.get("mirror_axes")))
    ...
    '''Here the dist map is moving to another key so it does not get intensity/Gaussian transform'''
    val_transforms.append(AppendChannelsTransform("data", "dist", [1], remove_from_input=True))
