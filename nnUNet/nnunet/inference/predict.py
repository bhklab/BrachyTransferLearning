...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False, mcdo: int = -1, fwd_passes: int = 50):
    """
    ...
    for preprocessed in preprocessing:
        ...
        if isinstance(d, str):
            ...
            os.remove(d)
            d = data
        # exclude dist map from prediction
        # input_channel = trainer.plans['num_modalities']
        # if d.shape[0] > input_channel:
        #     d = d[input_channel]
        ...
        if mcdo != -1:
            ...
            for fwd_pass in range(fwd_passes):
                softmax = trainer.predict_preprocessed_data_return_seg_and_softmax(
                    ...
                    mixed_precision=mixed_precision, mcdo=mcdo)[1]
                ...
                for p in params[1:]:
                    ...
                    softmax += trainer.predict_preprocessed_data_return_seg_and_softmax(
                        d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
                        step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
                        mixed_precision=mixed_precision, mdco=mcdo)[1]
                ...
                print('Finish predicting: ', str(fwd_pass))
                if fwd_pass == 0:
                    backgrounds = np.expand_dims(softmax[0,:], axis=0)
                    bladder = np.expand_dims(softmax[1,:], axis=0)
                    rectum = np.expand_dims(softmax[2,:], axis=0)
                    sigmoid = np.expand_dims(softmax[3,:], axis=0)
                    smallbowel = np.expand_dims(softmax[4,:], axis=0)
                else: 
                    backgrounds = np.vstack((backgrounds, np.expand_dims(softmax[0,:], axis=0)))
                    bladder = np.vstack((bladder, np.expand_dims(softmax[1,:], axis=0)))
                    rectum = np.vstack((rectum, np.expand_dims(softmax[2,:], axis=0)))
                    sigmoid = np.vstack((sigmoid, np.expand_dims(softmax[3,:], axis=0)))
                    smallbowel = np.vstack((smallbowel, np.expand_dims(softmax[4,:], axis=0)))

            '''entropy estimation'''
            # U_ts = -(np.mean(backgrounds, axis=0)*np.log(np.mean(backgrounds, axis=0)) + np.mean(bladder, axis=0)*np.log(np.mean(bladder, axis=0)) + 
            #         np.mean(rectum, axis=0)*np.log(np.mean(rectum, axis=0)) + np.mean(sigmoid, axis=0)*np.log(np.mean(sigmoid, axis=0)) +
            #         np.mean(smallbowel, axis=0)*np.log(np.mean(smallbowel, axis=0)))
            U_ts_B = -(np.mean(bladder, axis=0)*np.log(np.mean(bladder, axis=0)))
            U_ts_R = -(np.mean(rectum, axis=0)*np.log(np.mean(rectum, axis=0)))
            U_ts_S = -(np.mean(sigmoid, axis=0)*np.log(np.mean(sigmoid, axis=0)))
            U_ts_SB = -(np.mean(smallbowel, axis=0)*np.log(np.mean(smallbowel, axis=0)))
            # '''variance estimation'''
            # U_ts = np.mean(np.square(smallbowel),axis=0) - np.square(np.mean(smallbowel, axis=0))
            
            output_uncertainty_name = output_filename[:-7] + '_B_mcdo.nii.gz'
            seg_resized_itk = sitk.GetImageFromArray(U_ts_B.astype(np.float32))
            ...
            sitk.WriteImage(seg_resized_itk, output_uncertainty_name)

            output_uncertainty_name = output_filename[:-7] + '_R_mcdo.nii.gz'
            seg_resized_itk = sitk.GetImageFromArray(U_ts_R.astype(np.float32))
            seg_resized_itk.SetSpacing(dct['itk_spacing'])
            seg_resized_itk.SetOrigin(dct['itk_origin'])
            seg_resized_itk.SetDirection(dct['itk_direction'])
            sitk.WriteImage(seg_resized_itk, output_uncertainty_name)

            output_uncertainty_name = output_filename[:-7] + '_S_mcdo.nii.gz'
            seg_resized_itk = sitk.GetImageFromArray(U_ts_S.astype(np.float32))
            seg_resized_itk.SetSpacing(dct['itk_spacing'])
            seg_resized_itk.SetOrigin(dct['itk_origin'])
            seg_resized_itk.SetDirection(dct['itk_direction'])
            sitk.WriteImage(seg_resized_itk, output_uncertainty_name)

            output_uncertainty_name = output_filename[:-7] + '_SB_mcdo.nii.gz'
            seg_resized_itk = sitk.GetImageFromArray(U_ts_SB.astype(np.float32))
            seg_resized_itk.SetSpacing(dct['itk_spacing'])
            seg_resized_itk.SetOrigin(dct['itk_origin'])
            seg_resized_itk.SetDirection(dct['itk_direction'])
            sitk.WriteImage(seg_resized_itk, output_uncertainty_name)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None, disable_postprocessing: bool = True, mcdo: int = -1, fwd_passes: int = 50):
    """
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing, mcdo=mcdo, fwd_passes=fwd_passes)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = False
        else:
