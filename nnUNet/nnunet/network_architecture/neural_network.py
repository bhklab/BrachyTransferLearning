...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   verbose: bool = True, mixed_precision: bool = True, mcdo: int = -1) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        '''activate dropout during inference for uncertainty'''
        if mcdo != -1:
            def enable_dropout1(m):
                for each_module in m.modules():
                    if each_module.__class__.__name__.startswith('Dropout'):
                        each_module.train()

            def enable_dropout2(m):
                if "dropout" in m.__class__.__name__ or "Dropout" in m.__class__.__name__:
                    m.train()
            enable_dropout1(self)
            # self.apply(enable_dropout2)
