...
from nnunet.training.loss_functions.DP_loss import DisPenalizedCE
...
class Generic_UNet_DP(Generic_UNet):
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        super(Generic_UNet_DP, self).__init__(input_channels, base_num_features, num_classes, num_pool,
                                              ...
                                              max_num_features)
        self.dpce_loss = DisPenalizedCE()

    def forward(self, x, y=None, dist=None, return_hard_tp_fp_fn=False):
        ...
        if y is None:
            ...
            if self._deep_supervision and self.do_ds:
                dpce_losses = [self.dpce_loss(res[0], y[0], dist[0]).unsqueeze(0)]
                tps = []
                fps = []
                fns = []
                ...
                for i in range(1, len(y)):
                    dpce_losses.append(self.dpce_loss(res[i], y[i], dist[i]).unsqueeze(0))
                    res_softmax = softmax_helper(res[i])
                    tp, fp, fn, _ = get_tp_fp_fn_tn(res_softmax, y[i])
                    tps.append(tp)
                    fps.append(fp)
                    fns.append(fn)
                ret = dpce_losses, tps, fps, fns
            else:
                ce_loss = self.dpce_loss(res, y).unsqueeze(0)
